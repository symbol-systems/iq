...
// Prefer VFS when possible
try {
FileSystemManager fsManager = VFS.getManager();
FileObject root = fsManager.resolveFile(fileOrFolder.getAbsolutePath());
deploy(root);
return;
} catch (FileSystemException e) {
log.warn("VFS fallback to local file: {}", e.getMessage());
...
long startTime = System.currentTimeMillis();
log.info("deploy(VFS): " + fileOrFolder.getPublicURIString() + ", exists: " + fileOrFolder.exists());
if (!fileOrFolder.exists())
return;
if (fileOrFolder.isFile()) {
deployFile(fileOrFolder.getParent(), fileOrFolder);
} else {
if (fileOrFolder.getType().hasChildren()) {
FileObject[] files = fileOrFolder.getChildren();
for (FileObject child : files) {
if (child.isFile()) {
deployFile(fileOrFolder, child);
} else {
deploy(child);
}
}
...
long elapsedTime = (System.currentTimeMillis() - startTime) / 1000;
log.info("done(VFS): " + total_files + " in " + elapsedTime + "s (rdf: " + total_rdf_files + ", assets: "
+ total_asset_files + ") errors: " + total_errors);
}
...
IRI iri = Values.iri(getSelf() + file.getPublicURIString().substring(home.getPublicURIString().length()+1).replaceAll(Files.any_slash.pattern(), assetPathSeparator));
if (name.contains(".$.")) {
...
log.info("lake.file: {} @ {} --> {} & {}", iri, file.getPublicURIString(), file.getContent().getSize(), mediatype == null ? format : mediatype.getLocalName());
try (InputStream inStream = file.getContent().getInputStream()) {
