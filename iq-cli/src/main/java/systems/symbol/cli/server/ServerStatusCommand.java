...
if (port > 0) {
// New API may not support per-port health, fallback to runtime_type-level health
RuntimeStatus status = ServerRuntimeManagerFactory.getInstance().health(runtimeType);
ConsoleDisplay.getInstance().outf("status %s:%d => %s%n", runtimeType, port, status);
} else {
var all = ServerRuntimeManagerFactory.getInstance().listRuntimes(runtimeType);
if (all.isEmpty()) {
ConsoleDisplay.getInstance().outf("status %s => no active instances%n", runtimeType);
} else {
all.forEach((key, runtimeStatus) -> ConsoleDisplay.getInstance().outf("status %s => %s%n", key, runtimeStatus));
}
}
}
}
