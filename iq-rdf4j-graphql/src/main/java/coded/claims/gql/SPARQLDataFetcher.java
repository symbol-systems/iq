...
log.debug("sparql.select.query: " + prefixClauses + sparql);
...
where.append(subject).append(" rdf:type "+typeIRI+".\n");
...
if (isRequired(arg.getDefinition().getType())) {
String predIRI = GQL.toIRI(arg.getDefinition());
where.append("?").append(idField.getName()).append(" ").append(predIRI);
...
if (required) {
where.append(subject).append(" "+fieldIRI+" ?"+alias+".\n");
} else {
where.append("OPTIONAL { ").append(subject).append(" "+fieldIRI+" ?"+alias+" }.\n");
}
}
}
}
