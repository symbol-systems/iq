...
public GcpConnectorConfig(String apiKey, Duration pollInterval, String graphIri) {
this.apiKey = apiKey;
...
public static GcpConnectorConfig fromEnv() {
String apiKey = System.getenv("GCP_API_KEY");
String graphIri = System.getenv("GCP_GRAPH_IRI");
Duration poll = Duration.ofMinutes(5);
String interval = System.getenv("GCP_POLL_INTERVAL_SECONDS");
if (interval != null && !interval.isBlank()) {
try { poll = Duration.ofSeconds(Long.parseLong(interval)); } catch (NumberFormatException ignored) { }
}
return new GcpConnectorConfig(apiKey, poll, graphIri);
}
}
