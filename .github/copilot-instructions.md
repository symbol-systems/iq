...
- For scripts/queries use the `I_Contents` interface never hard-coded inside Java. Always load them via `IQScriptCatalog` or `ModelScriptCatalog` or `JarScriptCatalog`  and reference by name. This ensures they are versioned, testable, and can be overridden in different environments without code changes. Do not embed SPARQL queries or RDF templates directly in Java strings.
- Follow SOLID, DRY and clean code principles. Avoid code duplication, and prefer composition over inheritance. If you find yourself copying and pasting code, stop and refactor to extract common functionality into reusable methods or classes. 
- The goal is to maintain a clean, modular, and maintainable codebase that can evolve over time without becoming brittle or difficult to understand.
