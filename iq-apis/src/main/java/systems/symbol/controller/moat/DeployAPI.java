...
@Path("/moat/deploy")
public class DeployAPI extends RealmAPI {
...
log.info("ux.lake.deploy.lake: {} ", realm.getSelf());
...
FileObject file = platform.getInstance().getLake().resolveFile(_realm);
log.info("ux.lake.deploy.boot: {} --> {} == {}", file.getPublicURIString(), stopwatch, connection.size());
Lakes.boot(realm, file);
log.info("ux.lake.deploy.ok: {}", realm.getSelf().getLocalName());
