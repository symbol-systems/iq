...
public class BulkAssetLoaderTest {
...
assert loader.getIdentity().toString().equals(COMMONS.IQ_NS_TEST);
...
statements = tripleSource.getStatements(null, RDF.TYPE, null, loader.getIdentity());
...
MemoryRDFSRepository memoryRDFSRepository = new MemoryRDFSRepository();
SailRepositoryConnection connection = memoryRDFSRepository.getConnection();
BulkAssetLoader loader = new BulkAssetLoader(COMMONS.IQ_NS_TEST, connection);
loader.deploy(assetsRoot);
...
IRI iri = vf.createIRI("https://test.symbol.systems/cases#scripts/hello.groovy");
assert connection.hasStatement(iri, null, null, false, loader.getIdentity());
System.out.println("bulk.loader.scripts.found" );
IRI iri2 = vf.createIRI("https://test.symbol.systems/cases#hbs/index.hbs");
assert connection.hasStatement(iri2, null, null, false, loader.getIdentity());
