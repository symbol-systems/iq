...
public class RDFDump {
    ...
    public static Model dump(RepositoryConnection conn, GraphQueryResult from_result, RDFFormat format, IRI baseIRI)
            throws Exception {
        return dump(conn, from_result, new ByteArrayOutputStream(), format, baseIRI);
    }

    public static Model dump(RepositoryConnection conn, GraphQueryResult from_result, OutputStream out,
            RDFFormat format, IRI baseIRI) throws Exception {
    ...
    // public static Model ld(RepositoryConnection from, OutputStream out, IRI
    // baseIRI) throws Exception {
    // Model model = dmf.createEmptyModel();
    // copyNamespaces(from, model);
    // if (baseIRI != null) {
    // model.setNamespace("", baseIRI.stringValue());
