...
/**
 * a custom SPARQL function that calculates the distance between two
 * geo-coordinates
 */
public class GeoDistance extends CustomFunction {
    ...
    public Value evaluate(ValueFactory valueFactory, Value... args) throws ValueExprEvaluationException {
        if (args.length != 4 && args.length != 5) {
            throw new ValueExprEvaluationException(
                    "fn:GeoDistance(lon1,lat1,lon2,lat2,[inMiles]) requires 4 or 5 parameters");
        }
    ...
    // haversine distance (does not account for earth's elipsoidal shape. accuracy:
    // +/- 0.3%)
