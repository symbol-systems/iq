...
public class LocalAssetRepository extends RepositoryWrapper {
    private static final Logger log = LoggerFactory.getLogger( LocalAssetRepository.class );
    boolean loaded = false;

    public LocalAssetRepository() throws IOException {
        setDelegate(new SailRepository(new MemoryStore()));
    }

    public LocalAssetRepository(File root, String context) throws IOException {
        this();
        load(root, context);
    }

    public LocalAssetRepository(Repository delegated) {
    ...
    public IRI load(File assetsRoot, String context, boolean force, boolean deployRDF) throws IOException {
        ...
        try (RepositoryConnection connection = getConnection()) {
            BulkAssetLoader loader = new BulkAssetLoader(context, connection, force, deployRDF, true, true);
            loader.deploy(assetsRoot);
            connection.commit();
            connection.close();
            loaded = true;
            log.info("local.load.done: {} @ {}", assetsRoot.getAbsolutePath(), loader.getIdentity());
    ...
    public TripleSource getTripleSource(boolean inferred) {
        try (RepositoryConnection connection = this.getConnection()) {
            TripleSource tripleSource = new RepositoryTripleSource(connection, inferred);
            return tripleSource;
        }
    }
