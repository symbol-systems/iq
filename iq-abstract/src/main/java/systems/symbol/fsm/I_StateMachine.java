 ...
 *
 * FSMs represent systems with a finite number of states and transitions between these states. This interface
 * defines the contract that FSM implementations must adhere to IQ.
 *
 * The primary purpose of this interface is to provide methods for initializing the FSM, setting and retrieving
 * the initial state, getting the current state, retrieving possible transitions, registering state transition
 * listeners, performing state transitions, and checking the validity of transitions.
 *
 * The generic type parameter T represents the type of states in the FSM.
 *
 * Implementations of this interface should encapsulate the behavior and transitions of the FSM, ensuring
