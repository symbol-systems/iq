...
**Key design rules:**
1. **Order matters** — middleware is applied in the declared sequence; config in `mcp:pipeline` named graph controls order via `mcp:order` ***REMOVED***.
...
| `mcp:StripSecrets` | Remove any ***REMOVED*** matching `mcp:secretPattern` ***REMOVED*** |
...
| Transform IRI | Effect |
|---------------|--------|
| `mcp:RedactSecrets` | Remove ***REMOVED***s matching `mcp:secretPattern` ***REMOVED*** — defence in depth |
...
**Done when**: test that a result model containing a ***REMOVED*** matching `".*secret.*"` is scrubbed before the response leaves the pipeline.
...
config: mcp:upstreamUrl, mcp:toolPattern (***REMOVED***), mcp:mode ("forward"|"shadow"|"fanout")
```
...
**Vault secret interpolation**: `{mcp:vaultSecret:NAME}` tokens in RDF string ***REMOVED***s are resolved from `VFSPasswordVault` at startup — the secret value is never stored in the graph.
...
3. **Unit test — short-circuit**: trigger the blocking condition; verify adapter is NOT invoked and result contains appropriate `mcp:error` ***REMOVED***.
...
7. **No secret leakage**: grep test output and audit graph for ***REMOVED*** values matching `.*secret.*` / `.*password.*` — must be empty.
