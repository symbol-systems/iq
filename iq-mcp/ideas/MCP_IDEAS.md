...
```
If `iq:dryRun=true`: execute inside a transaction, capture delta (added/removed counts via `RepositoryResult` before/after), rollback — return delta without persisting. Otherwise commit. Log delta to `mcp:audit` as `mcp:added` / `mcp:removed` ***REMOVED***s.
...
```
Return subgraph as Turtle with `mcp:hopCount` ***REMOVED*** in result.
