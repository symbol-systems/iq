# MCP_V2.md — Idiomatic, Declarative MCP (Model Context Protocol) for IQ


**IQ as Claude's Knowledge + Action Fabric**: MCP bridges Claude's reasoning capabilities with IQ's neuro-symbolic fact graphs, SPARQL query engine, and agentic workflows. Claude becomes a **reasoning agent that operates over declarative, auditable RDF graphs** — not just a code executor.
...
- RDF role: `rdf:value` is used as the canonical literal container for message content, SPARQL text, or prompt templates (e.g., `:Message rdf:value "..."` or `.iq` resources containing SPARQL via `rdf:value`).
