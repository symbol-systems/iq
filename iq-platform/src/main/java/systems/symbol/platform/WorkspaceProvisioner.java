 ...
 * Each repository has an `iq.import.{repo}` property that includes the folder
 * to import from
 * Each file within the asset folder is imported (supports triples & text
 * content).
 *
...
Repository repository = ensureRepository ? this.workspace.alwaysGetRepository(name)
: this.workspace.getRepository(name);
if (repository == null) {
log.warn("workspace.deploy.repository.missing: {}", name);
return;
}
try (RepositoryConnection connection = repository.getConnection()) {
IQStore iq = new IQConnection(workspace.getSelf(), connection);
