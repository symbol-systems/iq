...
if (realm == null)
return new OopsResponse("api.ux.state.realm", Response.Status.NOT_FOUND).asJSON();
try {
...
if (repository == null)
return new OopsResponse("api.ux.state.repository", Response.Status.NOT_FOUND).asJSON();
...
return new OopsResponse("api.ux.update.realm", Response.Status.BAD_REQUEST).asJSON();
I_Realm realm = platform.getRealm(_realm);
if (realm == null)
return new OopsResponse("api.ux.update.realm", Response.Status.NOT_FOUND).asJSON();
DecodedJWT jwt;
try {
...
return new OopsResponse("api.ux.update.query", Response.Status.BAD_REQUEST).asJSON();

log.info("ux.update.jwt: {} --> {} -> {}", jwt.getSubject(), jwt.getAudience(), jwt.getIssuer());

Repository repository = realm.getRepository();
if (repository == null)
return new OopsResponse("api.ux.update.repository", Response.Status.NOT_FOUND).asJSON();
