 ...
 */
@Path("llm")
public class LLM extends GuardedAPI {
    /**
    ...
    public Response answer(@PathParam("model") String model,
        ...
        if (!Validate.isBearer(auth)) {
            log.info("api.llm.openai#protected");
        ...
        if (Validate.isNonAlphanumeric(repo)) {
            return new OopsResponse("api.llm.openai#repository-invalid", Response.Status.BAD_REQUEST).asJSON();
        }
