...
Major components
- `iq-run-apis` — Quarkus runtime exposing REST APIs (LLM endpoints, Agent APIs, Model APIs).
...
- Runtimes (Quarkus apps in `iq-run-apis`) load scripts and catalogs at startup, exposing APIs that query/modify the graph and call LLMs.
...
Quick commands
- Dev (live code): `./mvnw compile quarkus:dev -pl iq-run-apis -am` (or `./bin/iq`)
...
- `iq-run-apis/README.md` and `iq-run-apis/docs/API_LLM.md` (runtime and LLM API examples)
