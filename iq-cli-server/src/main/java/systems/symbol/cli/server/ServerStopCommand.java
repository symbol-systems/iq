...
boolean result = ServerRuntimeManagerFactory.getInstance().stop(runtimeType, port);
ConsoleDisplay.getInstance().out(String.format("stop %s%s => %s%n", runtimeType, port > 0 ? ":" + port : "", (result ? "OK" : "FAIL")));
}
}
